...
function SplashScreen({ onFinish }) {
  ...
  useEffect(() => {
    Animated.parallel([
      ...
      setTimeout(() => {
        Animated.timing(exitAnim, { toValue: 0, duration: 600, useNativeDriver: true }).start(onFinish);
      }, 1500);
    });
  }, []);

  return (
    <Animated.View style={{ opacity: exitAnim }} className="absolute inset-0 z-[10000] bg-black items-center justify-center">
      ...
      <Animated.View style={{ opacity: fadeAnim, transform: [{ scale: scaleAnim }] }} className="items-center">
        <Image source={require('./assets/logo.png')} className="w-48 h-20" resizeMode="contain" />
      </Animated.View>
...
function LoadingBar() {
  ...
  useEffect(() => {
    Animated.timing(widthAnim, { toValue: 200, duration: 2500, useNativeDriver: false }).start();
  }, []);
  return (
    <View className="absolute bottom-16 items-center">
      <View className="w-[200px] h-[2px] bg-white/10 rounded-full overflow-hidden">
        <Animated.View style={{ width: widthAnim }} className="h-full bg-merah rounded-full" />
      </View>
    </View>
  );
}
...
export default function App() {
  ...
  const isMainView = ['matches', 'standings', 'profile'].includes(currentView);
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      {currentView === 'matches' && (
        <MatchesScreen
          onMatchClick={(id) => navigateTo('detail', id)}
          onLeaguePress={(league) => navigateTo('league', league)}
        />
      )}

      {currentView === 'standings' && (
        <TeamScreen teamId={null} goBack={() => setCurrentView('matches')} />
      )}

      {currentView === 'detail' && selectedMatchId && (
        <MatchDetailScreen
      ...
      {currentView === 'team' && selectedTeamId && (
        <TeamScreen
      ...
      {currentView === 'league' && selectedLeague && (
        <LeagueScreen
      ...
      {isMainView && (
        <View className="absolute bottom-0 left-0 w-full bg-culos flex-row pb-6">
          <TouchableOpacity onPress={() => setCurrentView('matches')} className="flex-1 py-3 items-center gap-1">
          ...
          <TouchableOpacity onPress={() => setCurrentView('standings')} className="flex-1 py-3 items-center gap-1">
          ...
          <TouchableOpacity onPress={() => setCurrentView('profile')} className="flex-1 py-3 items-center gap-1">
            <Svg width="21" height="21" viewBox="0 0 21 21" fill="none">
