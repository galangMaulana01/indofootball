...
export default function MatchesScreen({ onMatchClick }) {
  ...
  return (
    <ScrollView className="flex-1 w-full mb-20 bg-equd" showsVerticalScrollIndicator={false}>
      ...
      {/* Banner image */}
      <View className="w-full h-56 m-4 overflow-hidden relative">
        <Image
          source={require('../../assets/vini.png')}
              ...
              return (
                ...
                >
                  ...
                  <View className="items-center ml-3">
                    <View className={`w-7 h-7 rounded items-center justify-center mb-1 ${String(match.homeScore) > String(match.awayScore) ? 'bg-yellow-600' : 'bg-white/10'}`}>
                      <Text className={`text-sm font-black ${String(match.homeScore) > String(match.awayScore) ? 'text-black' : 'text-white'}`}>{match.homeScore}</Text>
                    </View>
                    <View className={`w-7 h-7 rounded items-center justify-center ${String(match.awayScore) > String(match.homeScore) ? 'bg-yellow-600' : 'bg-white/10'}`}>
                      <Text className={`text-sm font-black ${String(match.awayScore) > String(match.homeScore) ? 'text-black' : 'text-white'}`}>{match.awayScore}</Text>
                    </View>
                  </View>
                </TouchableOpacity>
              );
